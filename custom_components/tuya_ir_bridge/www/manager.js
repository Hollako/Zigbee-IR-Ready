...
class ZigbeeIRPanel extends HTMLElement {
  ...
  render() {
    ...
    this.shadowRoot.innerHTML=`<style>${CSS}
      ...
      .command-name{font-weight:600;margin-bottom:7px}.command-toolbar{display:flex;align-items:end;gap:7px;flex-wrap:wrap}.command-inputs{display:flex;align-items:end;gap:7px;flex:1 1 430px}.command-inputs label{font-size:12px;display:grid;gap:4px}.command-inputs label:first-child{flex:0 1 170px}.command-inputs [data-bits]{flex:0 0 72px}.command-inputs [data-code]{flex:1 1 150px}.command-inputs input,.command-inputs select{box-sizing:border-box;width:100%;height:36px;padding:7px;border:1px solid var(--divider-color,#666);border-radius:5px;background:var(--card-background-color,#fff);color:var(--primary-text-color,#222)}.command-learned{align-self:center;font-size:12px;color:var(--secondary-text-color,#aaa)}.command-row details{margin:0;color:var(--secondary-text-color,#aaa)}.command-row details[open]{flex:1 0 100%;order:3}.command-row summary{cursor:pointer;box-sizing:border-box;height:36px;padding:9px 11px;border:1px solid var(--divider-color,#666);border-radius:5px;white-space:nowrap}.command-row details[open] summary{width:max-content;margin-bottom:7px}.command-actions{display:flex;gap:7px}.command-row button{height:36px;border:0;border-radius:5px;padding:7px 12px;background:var(--primary-color,#03a9f4);color:var(--text-primary-color,#fff)}@media(max-width:700px){.command-inputs{flex-basis:100%}.editor-tabs{gap:4px}}
