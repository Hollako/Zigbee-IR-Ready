...
export function mountEditor(panel,form,selected,values,capture) {
  ...
  function commandRow(section,name,command){
    const row=document.createElement('div');row.className='command-row';row.innerHTML=`<label>${esc(name)}<textarea data-command="${esc(name)}" aria-label="${esc(name)} command" placeholder='{"Bits":32,"Data":"0x20DF10EF"}'></textarea></label><div><button type="button" data-learn>Learn</button><button type="button" data-test>Test</button><button type="button" data-clear>Clear</button></div>`;
    ...
    const details=document.createElement('details');const summary=document.createElement('summary');summary.textContent='Advanced command JSON';details.append(summary);input.parentNode.after(simple);details.append(input);simple.after(details);
