...
(async()=>{
  ...
  try {
    ...
    await page.evaluate(async()=>{window.customCards=[];await import('/remote_card.js');document.body.style.setProperty('--primary-text-color','#e1e1e1');document.body.style.setProperty('--card-background-color','#1c1c1c');const card=document.createElement('zigbee-ir-ready-remote-card');document.body.replaceChildren(card);window.remoteCalls=[];card.setConfig({entity:'remote.tv',title:'Living Room TV'});card.hass={states:{'remote.tv':{state:'on',attributes:{friendly_name:'Living Room TV',zigbee_ir_device_id:'tv',configured_commands:['power','volume_up','up','ok','digit_1']}}},callService:async(...args)=>remoteCalls.push(args)};});
